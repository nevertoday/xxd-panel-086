<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 086 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 086 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、姿态、方向与叙事焦点 → 只保留 3–5 个关键信息 → 用剪影、几何形与少量结构重塑 → 根据方向和视觉重量偏置主体 → 建立一个焦点、一个方向和一片有意义的留白 → 从原图压缩 2–4 种专色 → 加入干刷、滚墨、拖墨、断墨和不均匀覆盖 → 让窄体文字与视觉轴线共同平衡画面
```

## 成品中最容易识别的风格特点

- 主体可偏置、缩小、裁切或靠近边缘，留白承担天空、距离、光线、海面、沉默或时间感。
- 剪影与简洁色块构成主体，环境只保留真正参与叙事的少数形态。
- 油墨表面保留横向拖墨、飞白、断墨和粗粝覆盖，拒绝光滑矢量与数字渐变。
- 配色压缩为鲜明主色、深色结构色、浅色纸基和可选点睛色，保留原图色相记忆。
- 窄体文字可贴近剪影、沿视觉轴线或地平线组织，成为版式重心而非附加说明。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-086-prompt.en.md)
...
| **[xxd-panel-086](https://github.com/nevertoday/xxd-panel-086)** | 中世纪现代主义 · 限色丝网版画 · 剪影几何 · 干刷拖墨 · 断墨飞白 · 有意义的留白 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–086） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
