<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 086 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 086 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

機械的な処理は要素を残しすぎ、テンプレート的なレトロ表現は装飾で画面を埋め、焦点・方向・意味のある余白を失いがちです。

**Panel 086** は画面を成立させる3〜5個の視覚情報だけを残し、シルエット、幾何形、簡潔な色面で中世紀モダニズムの限定色スクリーンプリントへ変換します。2〜4色の特色とドライブラシ、ローラー跡、引きずったインク、断墨、不均一な被覆が粗い印刷の呼吸を残します。

### 向いている用途

- 写真を3〜5個の決定的な情報へ絞り、遠くから読める強い版画構図にしたい場合
- シルエット、幾何形、限定色面、大きな余白で写真の意味を凝縮したい場合
- ドライブラシ、ローラー跡、横方向の擦れ、飛白、断墨の粗いインク感を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 086 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 中世紀モダニズム · 限定色スクリーンプリント · シルエット幾何 · ドライブラシの擦れ · 断墨と飛白 · 意味のある余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 余白・比率・焦点が先に立ち、近づくと粗い油墨の細部が見える版画ポスター |
| **識別しやすい特徴** | 3〜5個の視覚情報、シルエットと幾何形、2〜4色の特色、横方向の擦れと断墨 |
| **入力の尊重** | 主体・輪郭・姿勢・方向・空間関係を読み、画面成立に必要なものだけを残す |
...
```text
主体・輪郭・姿勢・方向・焦点を分析 → 3〜5個の核心情報だけを残す → シルエット・幾何形・色面で再構成 → 2〜4色の特色と紙地を組む → ドライブラシ・擦れ・断墨・飛白を加える → 一つの焦点、一つの方向、大きな余白で非対称に整える
```

## 完成作の特徴

- 主体はシルエット、幾何形、簡潔な色面で示し、環境は物語に必要な少数の形だけにする。
- 横方向の擦れ、飛白、断墨、不均一な被覆を残し、滑らかなベクターやデジタル勾配を避ける。
- 原写真の色相記憶を2〜4色の特色へ圧縮し、静かな紙地と組み合わせる。
- 一つの明確な焦点、一つの主方向、意味のある大きな余白を保つ。
- 細身で控えめな現代主義書体を、原文の規則に従って余白と視覚軸へ配置する。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-086-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 086 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-086 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-086 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-086 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-086 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 086 を適用し、共通設定は一度だけ解決
/xxd-panel-086 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-086`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-086-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-086-prompt.zh-CN.md)
...
| **[xxd-panel-086](https://github.com/nevertoday/xxd-panel-086)** | 中世紀モダニズム · 限定色スクリーンプリント · シルエット幾何 · ドライブラシ · 断墨 · 意味のある余白 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–086） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
