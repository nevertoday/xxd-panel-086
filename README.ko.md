<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 086 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 086이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

기계적 필터는 요소를 지나치게 많이 남기고, 템플릿식 복고 포스터는 장식으로 화면을 채워 초점·방향·의미 있는 여백을 잃기 쉽습니다.

**Panel 086**은 화면을 성립시키는 3~5개의 핵심 정보만 남기고, 실루엣·기하 형태·간결한 색면으로 미드센추리 모더니즘의 제한색 스크린 프린트로 변환합니다. 2~4색 별색과 드라이 브러시, 브레이어 질감, 끌어낸 잉크, 끊긴 잉크, 고르지 않은 피복이 거친 인쇄 호흡을 남깁니다.

### 이런 경우에 적합합니다

- 사진을 3~5개의 결정적 정보로 줄여 멀리서도 읽히는 강한 판화 구성을 원할 때
- 실루엣, 기하학, 제한된 색면과 넓은 여백으로 사진의 의미를 응축하고 싶을 때
- 드라이 브러시, 롤러 자국, 수평 끌림, 비백과 끊긴 잉크의 거친 질감을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 086로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 미드센추리 모더니즘 · 제한색 스크린 프린트 · 실루엣 기하 · 드라이 브러시 끌림 · 끊긴 잉크와 비백 · 의미 있는 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 여백·비율·초점이 먼저 읽히고 가까이서 거친 인쇄 디테일이 드러나는 판화 포스터 |
| **대표 특징** | 3~5개 핵심 정보, 실루엣과 기하 형태, 2~4색 별색, 끌리고 끊긴 잉크 |
| **입력 존중 방식** | 주제·윤곽·자세·방향·공간 관계를 읽어 화면에 필요한 것만 남김 |
...
```text
주제·윤곽·자세·방향·초점 분석 → 3~5개 핵심 정보만 보존 → 실루엣·기하 형태·색면으로 재구성 → 2~4색 별색과 종이 바탕 구성 → 드라이 브러시·끌림·끊김·비백 추가 → 하나의 초점·방향과 큰 여백으로 비대칭 정리
```

## 완성작의 특징

- 주제는 실루엣·기하 형태·간결한 색면으로 드러내고 환경은 서사에 필요한 소수의 형태만 남깁니다.
- 수평 끌림, 비백, 끊긴 잉크와 고르지 않은 피복을 유지하고 매끈한 벡터와 디지털 그라데이션은 피합니다.
- 원본의 색 기억을 2~4색 별색으로 압축하고 조용한 종이 바탕과 조합합니다.
- 하나의 명확한 초점, 하나의 주된 방향과 의미 있는 넓은 여백을 유지합니다.
- 글자는 원문 규칙에 따른 절제된 좁은 현대주의 서체로 여백과 시각 축에 맞춰 배치합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-086-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 086의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-086 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-086 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-086 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-086 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-086 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-086 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 086 적용, 공통 설정은 한 번만 확정
/xxd-panel-086 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-086`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-086-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-086-prompt.zh-CN.md)
...
| **[xxd-panel-086](https://github.com/nevertoday/xxd-panel-086)** | 미드센추리 모더니즘 · 제한색 스크린 프린트 · 실루엣 기하 · 드라이 브러시 · 끊긴 잉크 · 의미 있는 여백 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–086) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
